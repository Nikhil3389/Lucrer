...
const Market = memo(() => {
  ...
  const getCoinsHandler = async()=>{
    try {
      const data = await getAllCoinsPrice();
      console.log(data);
    } catch (err) {
      console.log(err);
    }
  ...
  const moreHandler = () => {
    if (!token) {
      toast.error('Please login to see more');
      return;
    }
    navigate('/capital');
  };
    ...
    <>
      ...
      <Box>
        ...
        <div className='box-button box-vertical-padding box-horizontal-padding'>
          <button className='button button-purple button-medium button-block' onClick={moreHandler}>
            More
